...
antlrcpp::Any FormatVisitor::visitVarDecl(LuaParser::VarDeclContext* ctx) {
    ...
    bool temp = _chopDownParameter;
    _chopDownParameter = false;
    ss << visitExplist(ctx->explist()).as<string>();
    _chopDownParameter = temp;
    if (ctx->SEMI() != NULL) {
...
antlrcpp::Any FormatVisitor::visitDoStat(LuaParser::DoStatContext* ctx) {
    ...
    stringstream ss;
    ss << ctx->DO()->getText();  //
    ss << visitBlockAndComment(ctx->DO(), ctx->block());
    ss << ctx->END()->getText();
    return ss.str();
}
...
antlrcpp::Any FormatVisitor::visitRepeatStat(LuaParser::RepeatStatContext* ctx) {
    ...
    ss << visitBlockAndComment(ctx->REPEAT(), ctx->block());
    ss << ctx->UNTIL()->getText()             //
...
antlrcpp::Any FormatVisitor::visitIfStat(LuaParser::IfStatContext* ctx) {
    ...
    if (ctx->ELSEIF().size() == 0 && ctx->ELSE() == NULL) {
        if (needKeepBlockOneLine(ctx->THEN().front(), ctx->block().front())) {
            ss << commentAfter(ctx->THEN().front(), " ");
            bool temp = _chopDownBlock;
            _chopDownBlock = false;
            ss << visitBlock(ctx->block().front()).as<string>();  //
            _chopDownBlock = temp;
            if (!isBlockEmpty(ctx->block().front())) {
                ss << commentAfter(ctx->block().front(), " ");
            }
            ss << ctx->END()->getText();
            return ss.str();
        }
...
antlrcpp::Any FormatVisitor::visitExplist(LuaParser::ExplistContext* ctx) {
    ...
    for (int i = 0; i < n; i++) {
        ss << ctx->COMMA()[i]->getText();
        if (_chopDownParameter) {
            ss << commentAfterNewLine(ctx->COMMA()[i], 0);
            ss << indent();
        } else {
            ss << commentAfter(ctx->COMMA()[i], " ");
        }
        ss << visitExp(ctx->exp()[i + 1]).as<string>();
        if (i != n - 1) {
    ...
    if (ctx->LP() != NULL) {
        ...
        if (ctx->explist() != NULL) {
            if (isParameterSimple(ctx->explist())) {
                ...
                ss << commentAfterNewLine(ctx->LP(), 1);
                ss << indent();
                bool temp = _chopDownParameter;
                _chopDownParameter = true;
                ss << visitExplist(ctx->explist()).as<string>();
                _chopDownParameter = temp;
                ss << commentAfterNewLine(ctx->explist(), -1);
                ss << indent();
                ss << ctx->RP()->getText();
            }
        } else {
...
antlrcpp::Any FormatVisitor::visitFuncbody(LuaParser::FuncbodyContext* ctx) {
    ...
    ss << ctx->RP()->getText();  //
    ss << visitBlockAndComment(ctx->RP(), ctx->block());
    ss << ctx->END()->getText();  //
    return ss.str();
}
...
antlrcpp::Any FormatVisitor::visitTableconstructor(LuaParser::TableconstructorContext* ctx) {
    ...
    if (ctx->fieldlist() != NULL) {
        if (isTableSimple(ctx)) {
            ...
            bool temp = _chopDownTable;
            _chopDownTable = false;
...
antlrcpp::Any FormatVisitor::visitFieldlist(LuaParser::FieldlistContext* ctx) {
    ...
    if (fields.size() > 1 || seps.size() > 0) {
        ...
        if (seps.size() == n && !config.extra_sep_at_table_end()) {
...
bool FormatVisitor::hasHardLineBreak(ParserRuleContext* ctx) {
    ...
    for (int i = l + 1; i < tokens.size() && i < r; i++) {
        auto token = tokens[i];
        if (token->getType() == LuaLexer::COMMENT) {
...
bool FormatVisitor::needKeepBlockOneLine(tree::ParseTree* previousNode, LuaParser::BlockContext* ctx) {
    if (!config.keep_simple_block_one_line()) {
        return false;
    }
    int stats = 0;
    for (auto& s : ctx->stat()) {
    ...
    // block too long
    if (ctx->getText().size() >= config.chop_down_block()) {
        return false;
    }
    ...
    string blockContent = visitBlock(ctx).as<string>();
...
string FormatVisitor::visitBlockAndComment(tree::ParseTree* previousNode, LuaParser::BlockContext* ctx) {
    stringstream ss;
    bool oneline = needKeepBlockOneLine(previousNode, ctx);
    if (oneline) {
        ss << commentAfter(previousNode, " ");
        bool temp = _chopDownBlock;
        _chopDownBlock = false;
        ss << visitBlock(ctx).as<string>();
        _chopDownBlock = temp;
        if (!isBlockEmpty(ctx)) {
            ss << commentAfter(ctx, " ");
        }
    } else {
        ss << commentAfterNewLine(previousNode, 1);
        bool temp = _chopDownBlock;
        _chopDownBlock = true;
        ss << visitBlock(ctx).as<string>();
        _chopDownBlock = temp;
        ss << commentAfterNewLine(ctx, -1);
        ss << indent();
    }
    return ss.str();
}
...
bool FormatVisitor::isTableSimple(LuaParser::TableconstructorContext* ctx) {
    ...
    bool temp = _chopDownTable;
    _chopDownTable = false;
    string fieldContext = visitFieldlist(ctx->fieldlist()).as<string>();
    _chopDownTable = temp;
